...
function setBadge(state) {
  ...
  const labels = { connecting: 'Đang kết nối…', online: 'Trực tuyến', offline: 'Mất kết nối' };
  badge.innerHTML = `<i class="fas fa-circle"></i> ${labels[state] || state}`;
}

// ── Sensor update ──
function updateSensors(data) {
  if (data.temperature !== undefined) setSensor('temp', data.temperature, SENSOR_THRESHOLDS.temp);
  if (data.humidity     !== undefined) setSensor('humi', data.humidity,    SENSOR_THRESHOLDS.humi);
  if (data.gas          !== undefined) setSensor('gas',  data.gas,         SENSOR_THRESHOLDS.gas);
...
function renderFaceLog(events) {
  ...
  if (events.length === 0) {
    list.innerHTML = '<p style="color:var(--text-dim);font-size:.8rem">Chưa có sự kiện nhận diện.</p>';
    return;
  }
  ...
  try {
    ...
    _sensorChart = new Chart(ctx, {
      ...
      options: {
        ...
        animation: false,
        plugins: {
          legend: { labels: { color: '#c9d1d9', boxWidth: 14, font: { size: 12 } } },
        },
        scales: {
          x: {
...
// ── Energy Report ──
const ENERGY_ICONS = { led: 'lightbulb', fan: 'fan', pump: 'tint', door: 'door-open' };
const ENERGY_NAMES = { led: 'Đèn LED', fan: 'Quạt', pump: 'Máy bơm', door: 'Cửa' };
...
async function pollEnergy() {
  ...
  try {
    ...
    container.innerHTML = ['led', 'fan', 'pump', 'door'].map(d => {
      const info   = data[d] || {};
